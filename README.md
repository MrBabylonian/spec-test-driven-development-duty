...
Load `spec-and-test-driven-development-duty`. It requires the sibling TDD, simplification, and review wrappers plus both packaged standards. Each wrapper loads its exact vendored source. The entrypoint and wrappers contain no host-specific runtime paths or commands.

The mandatory workflow is: obtain complete specifications/features → clarify every ambiguity → implement each requirement through vertical TDD → run `simplify-code` → run `requesting-code-review` → remediate and repeat cleanup/review until no blocker remains → issue the duty report. Applicable `AGENTS.md` files and every referenced file are re-read before every stage.
...
Version 0.2.0 does not auto-run or enforce policy through hooks. The host must load or invoke the entrypoint skill. Its verification gates then fail closed when required project evidence is unavailable.
...
`VERSION`, plugin manifests, the entrypoint, and portable wrappers carry package version `0.2.0`. Vendored sources retain their source versions when declared. `COMPONENTS.json` records wrapper-to-source relations, source identities, versions, companion files, copy mode, and digests. Change governed artifacts in one commit and release one tag. Roll back the package revision, not individual files.
